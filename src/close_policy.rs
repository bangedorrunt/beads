...
/// ADR-0001 §5.3 default policy, written by `br init` and applied whenever
/// `.beads/policy.yaml` is absent or `workflow.strict` is unset: closing is
/// fail-closed via the legal-close table; the five ADR-0019 verdict names
/// are NEVER demanded together (`require_all` of them is illegal in this
/// fork — `detect_illegal_require_all`).
pub const DEFAULT_FAIL_CLOSED_POLICY_YAML: &str = "\
...
pub struct GateRule {
    ...
    /// ADR-0001 §5.3 fail-closed close (beads_rust-fail-closed-close-rpay):
    /// when set on a `-> closed` transition, `evaluate_gates` runs
    /// `crate::verify::legal_close` against the recorded PASS rows for the
    /// transition instead of a static name list. Which gate names are legal
    /// is derived from the bead itself (priority, blast radius, AC shape,
    /// VERIFY runnability), so the YAML documents the chokepoint rather than
    /// listing five names.
    #[serde(default)]
...
pub fn detect_illegal_require_all(workflow: &Workflow) -> Vec<String> {
    ...
    for (key, rule) in &workflow.gates {
        if !rule.require_legal_close
            && rule.require_if.is_empty()
            && rule.require_all.len() == ADR0019_VERDICT_GATE_NAMES.len()
        {
            let mut names: Vec<&str> = rule
                .require_all
                .iter()
                .filter_map(|spec| match spec {
                    GateSpec::Named(name) => Some(name.as_str()),
                    GateSpec::MinReviewers(_) => None,
                })
                .collect();
            names.sort_unstable();
            let mut expected = ADR0019_VERDICT_GATE_NAMES;
...
fn legal_close_gate_names(input: &crate::verify::LegalCloseInput<'_>) -> Vec<&'static str> {
    crate::verify::VerdictKind::ALL
        ...
        .map(crate::verify::VerdictKind::gate_name)
        .collect()
}
...
/// ADR-0001 §5.3 fail-closed close: evaluate `require_legal_close` for a
/// `-> closed` transition. Close is authorized only when at least one
...
pub fn evaluate_require_legal_close(
    ...
    for result in results {
        let Some(kind) = crate::verify::VerdictKind::from_gate_name(&result.gate) else {
            continue; // non-verdict provider gates are out of scope here
        };
        // Only kinds that would legally authorize this close are relevant;
        // anything else recorded against the transition is loop noise.
        if !legal_names.contains(&kind.gate_name()) {
            continue;
        }
        if result.passed {
            pass_kinds.push(kind.gate_name());
        } else {
            return vec![PolicyViolation {
                gate: format!("gate_{}", kind.gate_name()),
                message: format!(
                    "transition '{from}' -> 'closed' is fail-closed for {issue_id}: a FAIL \
                     '{name}' row is recorded and a FAIL verdict can never authorize a close \
                     (ADR-0001 §5.3). Supersede it with a PASS from a legal verifier.",
                    name = kind.gate_name(),
                ),
                detail: Some(serde_json::json!({
                    "issue_id": issue_id,
                    "from": from,
                    "failing_verdict": kind.gate_name(),
                })),
            }];
        }
    }
    ...
    let names = if legal_names.is_empty() {
        "(none — no verdict kind is legal for this bead)".to_string()
    } else {
        legal_names.join(" | ")
    };
    vec![PolicyViolation {
        gate: "gate_require_legal_close".to_string(),
        message: format!(
            "transition '{from}' -> 'closed' is fail-closed for {issue_id}: no legal PASS gate \
             row is recorded. Legal gate names for this bead: {names}. Record one BEFORE \
             closing with `br gate report {issue_id} --gate <name> --provider <verifier> \
             --status pass --to closed`, then close with --commit-sha."
        ),
        detail: Some(serde_json::json!({
            "issue_id": issue_id,
            "from": from,
            "legal_gate_names": legal_names,
        })),
    }]
}
...
pub fn load_for_beads_dir(beads_dir: &Path) -> Result<PolicyDocument> {
    ...
    // ADR-0001 §5.3: `require_all` of the five ADR-0019 verdict names is
    // ILLEGAL in this fork — the legal-close table chooses the gate per bead;
    // demanding all five demands mutually exclusive proofs. Every command
    // that loads policy refuses such a file, so `br doctor` surfaces it.
    let illegal = detect_illegal_require_all(&document.workflow);
    if !illegal.is_empty() {
        return Err(BeadsError::Config(format!(
            "{} is illegal in this fork (ADR-0001 §5.3): transitions {illegal:?} use \
             require_all with all five ADR-0019 verdict names; replace with \
             `require_legal_close: true` and let legal_close choose the gate per bead",
            path.display(),
        )));
    }
